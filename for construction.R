...
residuals_mat <- get_res_matrix(confi_construction, cons_order, "x13")
residuals_mat
...
colnames(reco_fc)
reco_fc
setdiff(cons_order, colnames(reco_fc))
...
actuals   <- as.numeric(results_construction[[check_series]]$original)
residuals <- actuals - best_reco

# Plot ACF
acf(residuals, main = paste("Residuals ACF for", check_series))

lapply(confi_construction, function(x) head(x$original))
head(confi_construction$Confidence$original)
...
all_plots_list <- lapply(cons_order, function(v) {
  data.frame(
    Time = as.vector(time(results_construction[[v]]$original)),
    Actual = as.numeric(results_construction[[v]]$original),
    Reconciled = as.numeric(results_construction[[v]]$adjusted_str),
    Series = v
  )
})
...
all_plots_list <- lapply(cons_order, function(v) {
  ...
  data.frame(
    ...
    SA_adjusted = as.numeric(results_construction[[v]]$adjusted_str),
